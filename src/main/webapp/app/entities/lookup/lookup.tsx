...
export const Lookup = (props: ILookupProps) => {
  ...
  const { lookupList, match, loading, totalItems } = props;
  return (
    <div>
      ...
      <div className="table-responsive">
        {lookupList && lookupList.length > 0 ? (
                ...
                <tr key={`entity-${i}`}>
                  ...
                  <td className="text-right">
                    <div className="btn-group flex-btn-group-container">
                      ...
                      <Button
                        tag={Link}
                        to={`${match.url}/${lookup.id}/edit?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`}
                        color="primary"
                        size="sm"
                      >
                      ...
                      <Button
                        tag={Link}
                        to={`${match.url}/${lookup.id}/delete?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`}
                        color="danger"
                        size="sm"
                      >
          ...
          !loading && <div className="alert alert-warning">No Lookups found</div>
        )}
      </div>
      {props.totalItems ? (
        <div className={lookupList && lookupList.length > 0 ? '' : 'd-none'}>
